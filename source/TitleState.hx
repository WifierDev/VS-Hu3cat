...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		if (!initialized)
		{
			...
			if(FlxG.sound.music == null) {
				FlxG.sound.playMusic(Paths.music('GoKittyCat'), 0);
		...
		#if (desktop && MODS_ALLOWED)
		var path = "mods/" + Paths.currentModDirectory + "/images/logoBumpinHu3.png";
		//trace(path, FileSystem.exists(path));
		if (!FileSystem.exists(path)){
			path = "mods/images/logoBumpinHu3.png";
		}
		//trace(path, FileSystem.exists(path));
		if (!FileSystem.exists(path)){
			path = "assets/images/logoBumpinHu3.png";
		}
		//trace(path, FileSystem.exists(path));
		logoBl.frames = FlxAtlasFrames.fromSparrow(BitmapData.fromFile(path),File.getContent(StringTools.replace(path,".png",".xml")));
		#else
		
		logoBl.frames = Paths.getSparrowAtlas('logoBumpinHu3');
		#end
		*/
		
		logoBl = new FlxSprite(0, 800);
		logoBl.frames = Paths.getSparrowAtlas('logoBumpinHu3');
		...
		*/
		...
		FlxTween.tween(logoBl, {angle: 10}, 2.9, {ease: FlxEase.quadInOut, type: PINGPONG});
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			{
				case 2:
					createCoolText(['Mod por'], 15);
					addMoreText('NNM Team', 15);
				case 3:
					deleteCoolText();
					createCoolText(['Nao associados com'], -60);
				// credTextShit.text += '\npresent...';
	...
	function skipIntro():Void
	{
		if (!skippedIntro)
		{
			...
			FlxTween.tween(logoBl, {y: -60}, 1.5, {ease: FlxEase.expoInOut});
			});
		}
	}
}
