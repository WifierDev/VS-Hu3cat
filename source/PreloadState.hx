...
class PreloadState extends MusicBeatState
{
	...
	override function create()
	{
		...
		var menuBG:FlxSprite = new FlxSprite().loadGraphic(Paths.image('huecat/load/beeMovie'));
		menuBG.screenCenter();
		add(menuBG);
        ...
        if(FlxG.save.data != null) {
            if (FlxG.save.data.uselessUnlock != null)
                unlockedSongs[0] = FlxG.save.data.uselessUnlock;
				
            if (FlxG.save.data.drunkUnlock != null)
                unlockedSongs[1] = FlxG.save.data.drunkUnlock;
	...
	function cache()
	{
		...
		#end
		FlxG.switchState(new Top10BestScreensState());
	}
